...
public class FileHandler {
     ...
     */
    @Contract("_ -> new")
    public static @NotNull FileHandler at(String filePath) {
        return new FileHandler(Objects.requireNonNull(filePath));
    }
     ...
     */
    public FileHandler charset(Charset charset) {
        this.charset = charset;
        return this;
    }

    public FileHandler createIfNotExists() {
        if (Files.notExists(path)) {
            if (path.getParent() != null) Try.runThrowing(() -> Files.createDirectories(path.getParent())).orThrow();
            Try.ofThrowing(() -> Files.createFile(path)).orThrow();
        }
        return this;
    }

    public FileHandler createDirectoriesIfNeeded() {
        if (path.getParent() != null) Try.ofThrowing(() -> Files.createDirectories(path.getParent())).orThrow();
        return this;
    }
     ...
     * @return the current {@code FileHandler} instance.
     */
    public FileHandler write(@NotNull String content, boolean append) {
        StandardOpenOption mode = append ? StandardOpenOption.APPEND : StandardOpenOption.TRUNCATE_EXISTING;
        Try.runThrowing(() -> Files.writeString(path, content, charset, StandardOpenOption.CREATE, mode)).orThrow();
        return this;
    }
     ...
     * @return the current {@code FileHandler} instance.
     */
    public FileHandler write(@NotNull String content) {
     ...
     * @return the current {@code FileHandler} instance.
     */
    public FileHandler append(@NotNull String content) {
     ...
     */
    public FileHandler copyTo(String targetPath) {
     ...
     */
    public FileHandler moveTo(String targetPath) {
     ...
     */
    public FileHandler renameTo(String newFileName) {
        Path parent = path.getParent();
        Path newPath = (parent != null) ? parent.resolve(newFileName) : Paths.get(newFileName);

        Try.runThrowing(() -> Files.move(path, newPath, StandardCopyOption.REPLACE_EXISTING)).orThrow();

        return FileHandler.at(newPath.toString());
    }

    public FileHandler delete() {
