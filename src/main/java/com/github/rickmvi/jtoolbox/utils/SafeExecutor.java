...
public interface SafeExecutor<T> {
    ...
    /**
     * Executes the given {@link IntConsumer} with the provided integer value and handles
     * any exceptions that occur during its execution. If an exception is thrown, it
     * is logged along with the class name of the provided {@link IntConsumer}.
     *
     * @param <T> the type parameter, not used directly in this method but may be inferred from context
     * @param runnable the {@link IntConsumer} to execute; must not be {@code null}
    ...
    static <T> T attemptOrElseGet(
        ...
        try {
            ...
            Logger.error(message, t, t.getMessage(), args);
            return fallback.get();
        }
    }
